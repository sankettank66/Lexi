(function () {
  ...
  class BaseProvider {
    ...
    buildPrompt(action, text) {
      const prompts = {
        'fix-grammar': `You are a professional proofreader and grammar expert. Fix any grammar, spelling, punctuation, and word choice errors in the following text. Return ONLY the corrected text without any explanations, quotes, or additional formatting. Do not change the meaning or tone of the original text unless it contains errors.\n\nText: "${text}"`,
        'rephrase': `You are a professional writing assistant. Rephrase the following text to improve clarity, flow, and readability while preserving the original meaning and tone. Return ONLY the rephrased text without any explanations, quotes, or additional formatting.\n\nText: "${text}"`
      };
      return prompts[action] || prompts['fix-grammar'];
    }
